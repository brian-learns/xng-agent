"""Web research skill: deterministic live web search (xng / SearXNG) and
headless page browsing (browser-use primitives).
...
so any NOOA agent can opt in without importing this module::
    ...
    self.skills.activate(["xng.web"])  # loads self.web, visible to the model

``WebResearchSkill`` owns all browser state (shared headless session and its
lock). It does not inherit from Agent — it is a plain tool belt that can be
...
class WebResearchSkill(Skill):
    """Live web research tools. search_web(query, limit) returns SearXNG hits
    (title, url, snippet, published date). browse(url) opens the page in a
    shared headless browser and returns its visible text (truncated) with the
    title and the FINAL url after redirects; it only reads pages — no forms or
    clicks. Every browsed page is appended to self.findings."""
    ...
    async def aclose(self) -> None:
        """Kill the shared browser session. Call when the agent's run is over."""
        await self._kill_browser()
